...
# IDK I used https://codebeautify.org/python-formatter-beautifier for this formatting
...
rate_up4 = [
    ...
    "Diona",
    "Fischl",
]  # Characters that can be rate-up in 4-star pools
...
def pity_calc(pool_name, user_data, user_name, star):
    return conf[pool_name]["base"] + (
        conf[pool_name]["increase"]
        * (1 + user_data[user_name][star] - conf[pool_name]["start"])
        if user_data[user_name][star] >= conf[pool_name]["start"]
        else 0
    )
...
while iteration > 0:
    ...
    while True:
        while user_data["wishes"] > 0:
            ...
            if cdone < user_data["wanted_characters"]:
                if pull <= pity_calc("5char", user_data, "character", "5pity"):
                    ...
                    if (
                        random.randint(0, 1)
                        and not user_data["character"]["5guarantee"]
                    ):
                        if (
                            (user_data["radiance"] == 3)
                            or (user_data["radiance"] == 2 and random.randint(1, 4) < 4)
                            or (user_data["radiance"] <= 1 and random.randint(0, 1))
                        ):
                    ...
                    if (
                        random.randint(0, 1)
                        and not user_data["character"]["4guarantee"]
                    ):
                ...
                if pull <= pity_calc("5weap", user_data, "weapon", "5pity"):
                    ...
                    if (
                        ...
                        and not user_data["weapon"]["5guarantee"]
                    ):
        ...
        if (
            not user_data["wishes"]
            or cdone == user_data["wanted_characters"]
            and wdone == user_data["wanted_weapons"]
        ):

            if (
                cdone == user_data["wanted_characters"]
                and wdone == user_data["wanted_weapons"]
            ):
                total["completed"] += 1
...
with open(file_to_load, "r", encoding="utf-8") as f:
    user_data = json.load(f)
print(
    f"""Total C{user_data["wanted_characters"]-1}R{user_data["wanted_weapons"]}: {total["completed"]}
